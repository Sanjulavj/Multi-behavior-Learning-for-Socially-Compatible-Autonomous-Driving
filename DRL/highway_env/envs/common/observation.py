...
class KinematicObservation(ObservationType):
    ...
    FEATURES: List[str] = ['presence', 'x', 'y', 'vx', 'vy', "SVO"]
    ...
    def observe(self) -> np.ndarray:
        ...
        # Add ego-vehicle
        df = pd.DataFrame.from_records([self.observer_vehicle.to_dict(vehicle_type_HV = 2)])[self.features]
        # print("df")
        # print(df)
        # Add nearby traffic
        ...
        if close_vehicles:
            ...
            df = df.append(pd.DataFrame.from_records(
                [v.to_dict(origin, observe_intentions=self.observe_intentions, vehicle_type_HV = 1)
